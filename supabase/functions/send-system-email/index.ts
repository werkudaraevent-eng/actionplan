...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!providerConfig || !template) {
      return new Response(
        JSON.stringify({ success: false, message: 'Missing providerConfig or template' }),
    ...
    if (providerConfig.type === 'gmail') {
      ...
      if (!providerConfig.email || !providerConfig.appPassword) {
        return new Response(
          JSON.stringify({ success: false, message: 'Gmail requires email and appPassword' }),
          { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        )
      }
      ...
      if (!providerConfig.host || !providerConfig.port || !providerConfig.username || !providerConfig.password) {
        return new Response(
          JSON.stringify({ success: false, message: 'SMTP requires host, port, username, and password' }),
          { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        )
      }
      ...
      transporter = nodemailer.createTransport({
        ...
        tls: providerConfig.security === 'tls' ? { rejectUnauthorized: false } : undefined
      })
    }
    ...
    const fromEmail = providerConfig.type === 'gmail' 
