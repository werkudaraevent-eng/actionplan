-- Update the audit trigger to capture RCA (Root Cause Analysis) fields in new_value
-- This ensures gap_category, gap_analysis, and specify_reason are logged for "Not Achieved" status changes
...
BEGIN
  ...
  IF TG_OP = 'INSERT' THEN
    -- Check if this is a carry-over (has is_carry_over flag)
  ...
  IF TG_OP = 'UPDATE' THEN
    -- Build previous and new value objects with ALL relevant fields including RCA
    ...
    IF OLD.status IS DISTINCT FROM NEW.status THEN
      ...
      -- Build description based on new status
      IF NEW.status = 'Not Achieved' THEN
        -- Include RCA summary in description for Not Achieved
    ...
    -- ========================================
    -- PRIORITY 10: Remark Changes (only if explicitly changed by user)
    -- ========================================
...
COMMENT ON FUNCTION log_action_plan_changes() IS 'Enhanced Audit Trigger v3: Now captures RCA fields (gap_category, gap_analysis, specify_reason) in new_value for Not Achieved status changes. Handles all action plan changes with detailed logging.';;
