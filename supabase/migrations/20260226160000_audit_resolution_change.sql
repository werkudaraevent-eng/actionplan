-- ============================================================================
-- FIX: Audit log for resolution_type changes (Carry Over ↔ Drop)
-- ============================================================================
-- When a PIC switches from Carry Over to Drop (or vice versa), the status
-- stays 'Not Achieved', so the existing STATUS_UPDATE check never fires.
-- This migration adds a dedicated RESOLUTION_CHANGED audit trail.
-- ============================================================================

-- 1. Update the change_type constraint to include RESOLUTION_CHANGED
...
-- 2. Rebuild the audit trigger function with RESOLUTION_CHANGED support
...
BEGIN
  ...
  IF TG_OP = 'UPDATE' THEN
    ...
    v_prev_value := jsonb_build_object(
      ...
      'pic_ids', OLD.pic_ids,
      'resolution_type', OLD.resolution_type,
      'is_carry_over', OLD.is_carry_over,
      'is_drop_pending', OLD.is_drop_pending
    );
    ...
    v_new_value := jsonb_build_object(
      ...
      'is_carry_over', NEW.is_carry_over,
      'is_drop_pending', NEW.is_drop_pending
    );
    ...
    -- ========================================
    -- PRIORITY 4.5: Resolution Type Changed (Carry Over ↔ Drop)
    -- This fires when status stays 'Not Achieved' but follow-up action changes
    -- ========================================
    IF OLD.resolution_type IS DISTINCT FROM NEW.resolution_type THEN
      v_change_type := 'RESOLUTION_CHANGED';
      
      v_description := format('🔀 Follow-up action changed: %s → %s',
        CASE OLD.resolution_type
          WHEN 'carried_over' THEN 'Carry Over'
          WHEN 'dropped' THEN 'Drop/Cancel'
          ELSE COALESCE(OLD.resolution_type, 'None')
        END,
        CASE NEW.resolution_type
          WHEN 'carried_over' THEN 'Carry Over'
          WHEN 'dropped' THEN 'Drop/Cancel'
          ELSE COALESCE(NEW.resolution_type, 'None')
        END
      );
