-- Enhanced audit trigger with context-aware Alert resolution messages
-- Adds specific logging for Management actions on escalations

-- Drop existing trigger first
DROP TRIGGER IF EXISTS action_plan_audit_trigger ON action_plans;

-- Create the enhanced trigger function with Alert resolution handling
...
BEGIN
  ...
  IF TG_OP = 'UPDATE' THEN
    ...
    -- ========================================
    -- PRIORITY 1: Alert Status Change (Raising Alert)
    -- ========================================
    ...
    -- ========================================
    -- PRIORITY 2: Alert Resolution - Management Resolved (Alert -> On Progress)
    -- ========================================
    IF OLD.status = 'Alert' AND NEW.status = 'On Progress' THEN
      v_change_type := 'ALERT_RESOLVED';
      v_description := 'MANAGEMENT RESOLVED: Escalation marked as resolved. Action Plan returned to progress queue.';
    ...
    -- ========================================
    -- PRIORITY 3: Alert Resolution - Management Closed as Failed (Alert -> Not Achieved)
    -- ========================================
    IF OLD.status = 'Alert' AND NEW.status = 'Not Achieved' THEN
      v_change_type := 'ALERT_CLOSED_FAILED';
      v_description := 'MANAGEMENT DECISION: Escalation closed as Failed. Issue deemed unsolvable or target missed.';
    ...
    -- ========================================
    -- PRIORITY 4: Blocker Reason Updated (while in Alert)
    -- ========================================
    ...
    -- ========================================
    -- PRIORITY 5: Status Change (non-Alert transitions)
    -- ========================================
    IF OLD.status IS DISTINCT FROM NEW.status THEN
      ...
      IF OLD.outcome_link IS DISTINCT FROM NEW.outcome_link THEN
        log_details := log_details || '. Updated Evidence Link';
      END IF;
      ...
      IF OLD.evidence IS DISTINCT FROM NEW.evidence THEN
        log_details := log_details || '. Updated Evidence content';
      END IF;
    ...
    IF OLD.outcome_link IS DISTINCT FROM NEW.outcome_link THEN
      log_details := log_details || 'Updated Evidence Link (Proof of Evidence). ';
    END IF;
    ...
    IF OLD.blocker_reason IS DISTINCT FROM NEW.blocker_reason THEN
      log_details := log_details || format('Escalation Reason: "%s". ', 
        COALESCE(LEFT(NEW.blocker_reason, 80), 'Cleared'));
    END IF;
    
    -- Check Plan Details
    IF OLD.action_plan IS DISTINCT FROM NEW.action_plan THEN
      log_details := log_details || 'Updated Action Plan text. ';
    END IF;
    IF OLD.goal_strategy IS DISTINCT FROM NEW.goal_strategy THEN
      log_details := log_details || 'Updated Goal/Strategy. ';
    END IF;
    
    -- Check Grading fields
    IF OLD.quality_score IS DISTINCT FROM NEW.quality_score THEN
      IF NEW.quality_score IS NULL THEN
        log_details := log_details || format('Grade reset (was %s%%). ', OLD.quality_score);
      ELSE
        log_details := log_details || format('Graded with score: %s%%. ', NEW.quality_score);
      END IF;
    END IF;
    ...
    IF OLD.submission_status IS DISTINCT FROM NEW.submission_status THEN
      IF NEW.submission_status = 'submitted' THEN
        log_details := log_details || 'Submitted for review (locked). ';
      ELSIF NEW.submission_status = 'draft' THEN
        log_details := log_details || 'Recalled to draft (unlocked). ';
      END IF;
    END IF;
    ...
    IF OLD.unlock_status IS DISTINCT FROM NEW.unlock_status THEN
      IF NEW.unlock_status = 'pending' THEN
        log_details := log_details || format('Unlock requested: "%s". ', 
          COALESCE(LEFT(NEW.unlock_reason, 60), 'No reason'));
      ELSIF NEW.unlock_status = 'approved' THEN
        log_details := log_details || 'Unlock approved by Admin. ';
      ELSIF NEW.unlock_status = 'rejected' THEN
        log_details := log_details || format('Unlock rejected: "%s". ', 
          COALESCE(LEFT(NEW.unlock_rejection_reason, 60), 'No reason'));
      END IF;
    END IF;
...
COMMENT ON FUNCTION log_action_plan_changes() IS 'Enhanced Super Trigger with Alert Resolution handling: Logs ALL changes to action_plans with detailed field-level descriptions. Handles: INSERT (CREATED/CARRY_OVER), Alert workflow (ALERT_RAISED/ALERT_RESOLVED/ALERT_CLOSED_FAILED/BLOCKER_UPDATED), status changes (STATUS_UPDATE), evidence/outcome changes (OUTCOME_UPDATE), remark changes (REMARK_UPDATE), plan text changes (PLAN_DETAILS_UPDATED), grading (APPROVED/GRADE_RESET), submission workflow (SUBMITTED_FOR_REVIEW), and unlock workflow (UNLOCK_REQUESTED/APPROVED/REJECTED).';;
