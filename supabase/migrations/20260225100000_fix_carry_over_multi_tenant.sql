-- ============================================================================
-- FIX: Carry-over RPCs — Multi-tenant system_settings lookup
-- ============================================================================
-- The carry_over_plan(), resolve_and_submit_report(), reject_drop_request(), 
-- and reject_drop_request_v2() functions all queried:
--     SELECT ... FROM system_settings WHERE id = 1
--
-- But the multi-tenant migration (20260224043400) changed system_settings to
-- use company_id as the unique key and dropped the single-row constraint.
-- For any company whose settings row has id != 1, this query returns NULL,
-- causing carry-over penalties to silently use hardcoded defaults or fail.
--
-- FIX: Look up settings using the plan's company_id instead of id = 1.
-- ============================================================================
...
BEGIN
  ...
  -- 2. Fetch penalty settings (multi-tenant: use plan's company_id)
  --    If company_id is NULL (pre-multi-tenancy data), fall back to first settings row
  ...
  -- Fallback: try any settings row if company-specific lookup failed
  ...
  -- Final fallback defaults if no settings exist at all
  IF v_penalty_1 IS NULL THEN
    v_penalty_1 := 80;
    v_penalty_2 := 50;
  END IF;
...
BEGIN
  ...
  -- 1. Fetch penalty settings (multi-tenant: use plan's company_id)
  --    If company_id is NULL (pre-multi-tenancy data), fall back to first settings row
...
-- ============================================================
-- 3. reject_drop_request() — uses auto carry-over on closed reports
-- ============================================================
...
-- ============================================================
-- 4. reject_drop_request_v2() — uses auto carry-over on closed reports
-- ============================================================
