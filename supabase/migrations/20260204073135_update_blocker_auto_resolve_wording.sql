...
BEGIN
  ...
  IF TG_OP = 'UPDATE' THEN
    ...
    -- PRIORITY 3: Blocker Cleared (including auto-resolve on task closed)
    IF NEW.is_blocked = FALSE AND OLD.is_blocked = TRUE THEN
      IF (NEW.status = 'Achieved' OR NEW.status = 'Not Achieved') AND OLD.status IS DISTINCT FROM NEW.status THEN
        ...
        v_description := '• Status changed from "' || COALESCE(OLD.status, 'None') || '" to "' || COALESCE(NEW.status, 'None') || '"' || E'\n' || '• Blocker auto-resolved (task closed)';
      ELSE
    ...
    IF OLD.status IS DISTINCT FROM NEW.status THEN
      ...
      IF OLD.outcome_link IS DISTINCT FROM NEW.outcome_link THEN log_details := log_details || E'\n' || '• Updated Evidence Link'; END IF;
      IF OLD.remark IS DISTINCT FROM NEW.remark THEN log_details := log_details || E'\n' || '• Updated Remark: "' || COALESCE(LEFT(NEW.remark, 80), 'Cleared') || '"'; END IF;
      v_change_type := 'STATUS_UPDATE';
    ...
    IF OLD.outcome_link IS DISTINCT FROM NEW.outcome_link THEN log_details := log_details || '• Updated Evidence Link' || E'\n'; END IF;
    IF OLD.remark IS DISTINCT FROM NEW.remark THEN log_details := log_details || '• Updated Remark: "' || COALESCE(LEFT(NEW.remark, 80), 'Cleared') || '"' || E'\n'; END IF;
    IF OLD.blocker_reason IS DISTINCT FROM NEW.blocker_reason AND (NEW.is_blocked IS NOT DISTINCT FROM OLD.is_blocked) AND (NEW.status IS NOT DISTINCT FROM OLD.status) THEN log_details := log_details || '• Updated Blocker/Escalation details' || E'\n'; END IF;
    IF OLD.action_plan IS DISTINCT FROM NEW.action_plan THEN log_details := log_details || '• Updated Action Plan text' || E'\n'; END IF;
    IF OLD.goal_strategy IS DISTINCT FROM NEW.goal_strategy THEN log_details := log_details || '• Updated Goal/Strategy' || E'\n'; END IF;
    IF OLD.quality_score IS DISTINCT FROM NEW.quality_score THEN
      IF NEW.quality_score IS NULL THEN log_details := log_details || '• Grade reset (was ' || OLD.quality_score || '%)' || E'\n';
      ELSE log_details := log_details || '• Graded with score: ' || NEW.quality_score || '%' || E'\n'; END IF;
    END IF;
    IF OLD.submission_status IS DISTINCT FROM NEW.submission_status THEN
      IF NEW.submission_status = 'submitted' THEN log_details := log_details || '• Submitted for review (locked)' || E'\n';
      ELSIF NEW.submission_status = 'draft' THEN log_details := log_details || '• Recalled to draft (unlocked)' || E'\n'; END IF;
    END IF;
    IF OLD.unlock_status IS DISTINCT FROM NEW.unlock_status THEN
      IF NEW.unlock_status = 'pending' THEN log_details := log_details || '• Unlock requested' || E'\n';
      ELSIF NEW.unlock_status = 'approved' THEN log_details := log_details || '• Unlock approved by Admin' || E'\n';
      ELSIF NEW.unlock_status = 'rejected' THEN log_details := log_details || '• Unlock rejected' || E'\n'; END IF;
    END IF;
